...
def train_model(model, train_loader, val_loader, num_epochs=100, learning_rate=5e-4, alpha=1.0, weight_decay=1e-5, patience=200):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels, cell_locations) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}")):
            ...
            count_loss = criterion(cell_count.squeeze(1), labels).mean()
...
def main():
    ...
    # Set use_batch_norm to True or False as desired
    use_batch_norm = True  
    model = CellCounter(use_batch_norm=use_batch_norm)
    ...
    print("Training complete. Best model saved in 'Experiments/38/best_model.pth'")
