...
def train_model(model, train_loader, val_loader, num_epochs=100, learning_rate=1e-3):
    ...
    Returns:
        ...
        train_metrics, val_metrics (dict): Dict of lists of metrics (MAE, RMSE, ACP, Accuracy) for training and validation for each epoch.
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            # Count cells accuracy (new metric)
            predicted_cells = torch.round(outputs)  # Round to the nearest integer count of cells
            actual_cells = torch.round(labels)
            correct_cells = torch.sum(predicted_cells == actual_cells).item()
            total_correct_cells += correct_cells
            total_predicted_cells += labels.size(0)  # Assuming one label per image
        ...
        train_accuracy = total_correct_cells / total_predicted_cells * 100  # Accuracy as percentage
        ...
        val_loss, val_mae, val_rmse, val_acp, val_accuracy = evaluate_model(model, val_loader, criterion, device)
...
def evaluate_model(model, data_loader, criterion, device):
    ...
    Returns:
        ...
        accuracy (float): Accuracy of predicted cell counts.
    '''
    model.eval()
    total_loss = 0.0
    total_mae, total_rmse, total_acp = 0.0, 0.0, 0.0
    ...
    with torch.no_grad():
        for inputs, labels in data_loader:
            ...
            correct_cells = torch.sum(predicted_cells == actual_cells).item()
            total_correct_cells += correct_cells
            total_predicted_cells += labels.size(0)
    ...
    accuracy = total_correct_cells / total_predicted_cells * 100
    return total_loss / len(data_loader), total_mae / num_samples, total_rmse / num_samples, total_acp / num_samples, accuracy
